        ...
        static int Main(string[] args)
        {
            try
            {
                ...
                using (CultureInfoUtils.ChangeCultureUI("en"))
                {
                    ...
                    Console.WriteLine("..:: Welcome to AtTest Loading Application ::..");
                    ...
                    if (args.Any())
                    {
                        switch (args.First().ToLower().Trim('-', '/'))
                        {
                            ...
                            case "csharp": AtTestMigrations.CSharpMigrations(true); return 0;
                    ...
                    while (true)
                    {
                        Action? action = new ConsoleSwitch<string, Action>
                        {
                            ...
                            {"CS", () => AtTestMigrations.CSharpMigrations(false), "C# Migrations"},
                            {"S", Synchronize},
                            {"L", () => Load(null), "Load"},
                        }.Choose();
